...
//				// End AspectJ Extension
	...
	*/
	...
	protected final MethodBinding mostSpecificMethodBinding(MethodBinding[] visible, int visibleSize, TypeBinding[] argumentTypes, final InvocationSite invocationSite, ReferenceBinding receiverType) {
		...
		nextSpecific: for (MethodBinding current : moreSpecific) {
			if (current != null) {
				...
				for (MethodBinding next : moreSpecific) {
					if (next == null || current == next) {
						continue;
					}
					arik2 = next.original();
					if (original.getrb() == arik2.getrb()) // AspectJ Extension - was (original.declaringClass == original2.declaringClass)
					...
					if (!original.isAbstract()) {
						...
						if (current.hasSubstitutedParameters() || original.typeVariables != Binding.NO_TYPE_VARIABLES) {
							...
							arik2 = original.findOriginalInheritedMethod(arik2);
							if (arik2 == null)
								continue nextSpecific; // current's declaringClass is not a subtype of next's declaringClass
							// AspectJ Extension End
							if (!environment().methodVerifier().isParameterSubsignature(original, arik2))
								continue nextSpecific; // current does not override next
						}
					} else if (receiverType != null) { // should not be null if original isAbstract, but be safe
						TypeBinding superType = receiverType.findSuperTypeOriginatingFrom(original.getrb().erasure());// AspectJ Extension - was declaringClass, now getOwningClass()
...
//						superType = receiverType.findSuperTypeOriginatingFrom(original2.declaringClass.erasure());
						superType = receiverType.findSuperTypeOriginatingFrom(arik2.getrb().erasure());// AspectJ Extension - was declaringClass, now getOwningClass()
						if (arik2.declaringClass == superType || !(superType instanceof ReferenceBinding)) {
							// keep original2
						} else {
							// must find inherited method with the same substituted variables
							MethodBinding[] superMethods = ((ReferenceBinding) superType).getMethods(arik2.selector, argumentTypes.length);
							for (int m = 0, l = superMethods.length; m < l; m++) {
								if (superMethods[m].original() == arik2) {
									arik2 = superMethods[m];
									break;
								}
							}
						}
						if (original.typeVariables != Binding.NO_TYPE_VARIABLES)
							arik2 = original.computeSubstitutedMethod(arik2, environment());
						if (arik2 == null || !original.areParameterErasuresEqual(arik2))
							continue nextSpecific; // current does not override next
						if (original.returnType != arik2.returnType) {
							if (next.original().typeVariables != Binding.NO_TYPE_VARIABLES) {
								if (original.returnType.erasure().findSuperTypeOriginatingFrom(arik2.returnType.erasure()) == null)
						...
						if (shouldIntersectExceptions && arik2.declaringClass.isInterface()) {
